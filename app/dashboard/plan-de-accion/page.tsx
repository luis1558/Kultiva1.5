...
export default function PlandeaccionPage() {
  ...
  const dimensionesLinks: { [key: string]: string } = {
    "Calidad de vida": "/dashboard/plan-de-accion/calidad-de-vida",
    Compromiso: "/dashboard/plan-de-accion/compromiso",
    Comunicación: "/dashboard/plan-de-accion/comunicacion",
    "Cultura organizacional":
      "/dashboard/plan-de-accion/cultura-organizacional",
    "Diversidad e inclusión":
      "/dashboard/plan-de-accion/diversidad-e-inclusion",
    "Flexibilidad y autonomía":
      "/dashboard/plan-de-accion/flexibilidad-y-autonomia",
    "Gestión del cambio": "/dashboard/plan-de-accion/gestion-del-cambio",
    Liderazgo: "/dashboard/plan-de-accion/liderazgo",
    "Seguridad psicológica": "/dashboard/plan-de-accion/seguridad-psicologica",
    "Seguridad y estabilidad":
      "/dashboard/plan-de-accion/seguridad-y-estabilidad",
    "Ética e integridad": "/dashboard/plan-de-accion/etica-e-integridad",
    "Salud mental": "/dashboard/plan-de-accion/salud-mental",
  };

  useEffect(() => {
    const fetchDimensiones = async () => {
      try {
        const response = await fetch(`${baseURL}/api/plandeaccion`);
        if (!response.ok) {
    ...
    const checkPlan = async () => {
      try {
        const response = await fetch(`${baseURL}/api/check-plan`);
        if (!response.ok) {
  ...
  const handleDownloadAndSendEmail = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/api/correos`, {
        method: "POST",
        headers: {
      ...
      window.location.href = `${baseURL}/api/download-plan`; // Asegúrate de que esta ruta sea correcta
    } catch (error) {
