...
export default function CalidadDeVida() {
  ...
  const dimension = "Calidad de vida"; // Cambia este valor según la dimensión deseada
    ...
    const fetchSugerencias = async () => {
      try {
        const response = await fetch(
          `/api/sugerencia-gerencia?dimension=${encodeURIComponent(dimension)}`
        );
        if (!response.ok) {
          throw new Error(`Error: ${response.status}`);
        }
        const data = await response.json();
        const sugerenciasArray = Object.values(data).filter(
          (s) => s
        ) as string[];
