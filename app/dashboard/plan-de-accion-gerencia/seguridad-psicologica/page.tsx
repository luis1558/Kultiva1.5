...
const obtenerDimensionId = (dimension: string): number | null => {
  const dimensiones: Record<string, number> = {
    ...
    "Calidad de vida": 2,
    "Compromiso": 3,
    "Seguridad psicológica": 4, // ID correspondiente para Seguridad psicológica
    "Comunicación": 5,
...
export default function SeguridadPsicologica() {
  ...
  const dimension = "Seguridad psicológica"; // Dimensión deseada
  ...
  useEffect(() => {
    const fetchSugerencias = async () => {
      try {
        const response = await fetch(`/api/sugerencia-gerencia?dimension=${encodeURIComponent(dimension)}`);
        if (!response.ok) {
    ...
    const fetchPlanDeAccion = async () => {
      ...
      try {
        const response = await fetch(
          `/api/obtener-plan-gerencia?dimension_id=${dimensionId}`
        );
        if (response.ok) {
          const data = await response.json();
          ...
          if (data.sugerencia_1 || data.sugerencia_2) {
            setGuardado(true); // Si ya hay datos guardados, deshabilita el botón y los inputss
          }
        }
      } catch (error) {
  ...
  const guardarPlanDeAccion = async () => {
    if (!dimensionId) {
      setMensaje("La dimensión es inválida.");
      return;
    }
    const payload = {
    ...
    try {
      const response = await fetch("/api/guardar-plan-gerencia", {
        method: "POST",
